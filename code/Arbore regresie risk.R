...
library(tree)
library(ISLR)
hist(date_n$risk_score)
#Vom aplica logaritm asupra salariului pentru a il face distribuit normal
date_n$risk_score <-log(date_n$risk_score)
hist(date_n$risk_score)
library(caret)
split<-createDataPartition(y=date_n$risk_score, p=0.5, list=FALSE)
split
train <-date_n[split,]
train
test<-date_n[-split,]
test
arbore <-tree(risk_score~., train)
plot(arbore)
